...
Path.prototype.abs = function () {
  walkThroughCommands(this.content, function (command, x, y) {
    if (command.relative) {
...
Path.prototype.matrix = function (a, b, c, d, e, f) {
  for (var i=0; i < 6; i++) {
  ...
  walkThroughCommands(this.content, function (command, x, y) {
    ...
    if (command.type === 'A') {
      replacement = convertArc(command, x, y)
      replacement.forEach(function (cmd) {
...
function walkThroughCommands (commands, callback) {
  ...
  function replace (i, replacement) {
    commands.splice.bind(commands, i, 1).apply(commands, replacement)
